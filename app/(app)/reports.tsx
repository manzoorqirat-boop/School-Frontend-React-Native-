            ...
            {tabs.map(tb => {
              ...
              return (
                <TouchableOpacity key={tb.key} onPress={() => setTab(tb.key)}
                  ...
                  <Ionicons name={tb.icon} size={14} color={on ? '#fff' : colors.slate} />
                  <Text style={[styles.tabText, on && { color: '#fff' }]} numberOfLines={1}>{tb.label}</Text>
                </TouchableOpacity>
              );
            })}
          </View>
...
function AttendanceReports({ classes, sections, accent }: { classes: string[]; sections: string[]; accent: string }) {
  ...
  const run = useCallback(async () => {
    ...
    try {
      ...
      else Alert.alert('Class report failed', (cr.reason as any)?.message);
  ...
  async function doExport() {
    if (!rows?.length) { Alert.alert('Nothing to export', 'Run the report first.'); return; }
    try {
      ...
      Alert.alert('Exported', 'Choose where to send the attendance report.');
...
function FeeReports({ classes, accent }: { classes: string[]; accent: string }) {
  ...
  const run = useCallback(async () => {
    ...
    try {
      ...
      if (s.status === 'rejected' && c.status === 'rejected' && o.status === 'rejected')
        Alert.alert('Reports failed', (s.reason as any)?.message);
    } finally { setLoading(false); }
  }, [from, to, fCls]);
  ...
  async function exportOutstanding() {
    ...
    if (!items.length) { Alert.alert('Nothing to export', 'No outstanding invoices.'); return; }
    try {
      ...
      Alert.alert('Exported', 'Choose where to send the outstanding report.');
    } catch (e: any) { Alert.alert('Export failed', e.message); }
  }

  async function exportCollectionHtml() {
    const rows = collection?.byMethod ?? [];
    if (!rows.length) { Alert.alert('Nothing to export', 'No collections in this range.'); return; }
    try {
      ...
      Alert.alert('Exported', 'Choose where to send the collection report.');
    } catch (e: any) { Alert.alert('Export failed', e.message); }
  }
...
function StaffReports({ accent }: { accent: string }) {
  ...
  const run = useCallback(async () => {
    ...
    try {
      ...
      else Alert.alert('Report failed', (r.reason as any)?.message);
  ...
  async function doExport() {
    if (!teachers.length) { Alert.alert('Nothing to export', 'No staff attendance for this month.'); return; }
    try {
      ...
      Alert.alert('Exported', 'Choose where to send the staff report.');
    } catch (e: any) { Alert.alert('Export failed', e.message); }
  }
...
const styles = StyleSheet.create({
  ...
  tab: { flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 4,
    paddingVertical: 10, paddingHorizontal: 4, borderRadius: radius.md, backgroundColor: colors.surface,
