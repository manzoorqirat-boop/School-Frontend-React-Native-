...
export default function Privileges() {
  ...
  const load = useCallback(async () => {
    ...
    } catch (e: any) { toast.error('Could not load privileges', e.message); }
  ...
  async function toggle(priv: string, role: string) {
    ...
    try {
      ...
      // A brief per-row tick confirms the write landed. A toast per toggle
      // would be far too noisy on a matrix this size.
      setSavedKey(priv);
  ...
  async function resetRole(role: string) {
    Alert.alert('Reset role', `Reset all "${roleTheme[role]?.label ?? role}" privileges to defaults?`, [
      ...
      { text: 'Reset', style: 'destructive', onPress: async () => {
        try { const r = await API.post(`/api/privileges/role/${encodeURIComponent(role)}/reset`); toast.success('Role reset', `${r.privilegesTouched} privilege(s) restored to defaults.`); load(); }
  ...
  async function resetAll() {
    Alert.alert('Reset all', 'Reset the entire privilege matrix to defaults?', [
      ...
      { text: 'Reset All', style: 'destructive', onPress: async () => {
        try { await API.post('/api/privileges/reset-all'); toast.success('Matrix reset', 'All privileges restored to defaults.'); load(); }
        catch (e: any) { toast.error('Reset failed', e.message); }
      }},
    ]);
  }
    ...
    <Screen title={t('nav.privileges', 'Privileges')}
      ...
      <ScrollView contentContainerStyle={{ padding: spacing.lg }}>
        ...
        <View style={styles.banner}>
          <Ionicons name={editable ? 'information-circle' : 'lock-closed'} size={16}
            color={editable ? colors.info : colors.muted} />
          <Text style={styles.bannerText}>
            {editable
              ? 'Tap any role tag to grant or revoke. Each change saves to the server immediately — there is no separate Save step. The amber dot marks a privilege that differs from defaults.'
              : 'You can view the privilege matrix but not change it. Editing requires the "user:manage" privilege (School Admin or Superadmin).'}
          </Text>
        </View>
        {/* Role reset chips */}
        {editable && (
