...
function Subjects({ classes, academicYear, editable, accent }: {
  ...
  function remove(s: any) {
    // Alert.alert is the confirm idiom on native — the web build's promise-based
    // confirm() helper does not exist in this repo.
    Alert.alert('Delete subject',
      `Remove "${s.name}" from class ${cls}? Exams already created keep their copy of the subject.`, [
  ...
  function seed() {
    ...
    if (toAdd.length === 0) { toast.warning('Nothing to add', 'All common subjects already exist for this class.'); return; }

    Alert.alert('Add common subjects',
      `Add ${toAdd.length} subject(s) to class ${cls}:\n\n${toAdd.join(', ')}\n\nYou can edit or delete any of them afterwards.`, [
      { text: 'Cancel', style: 'cancel' },
        ...
        <Card>
          ...
          {items.length === 0 ? (
            ...
            <View key={s._id} style={styles.row}>
              ...
              {editable && (
                <>
                  <TouchableOpacity onPress={() => openEdit(s)} style={styles.iconBtn} hitSlop={6}>
                    <Ionicons name="create-outline" size={19} color={colors.slate} />
                  </TouchableOpacity>
                  <TouchableOpacity onPress={() => remove(s)} style={styles.iconBtn} hitSlop={6}>
...
function Scales({ editable, accent }: { editable: boolean; accent: string }) {
  ...
  function remove(s: any) {
    Alert.alert('Delete grading scale',
      `Remove "${s.name}"? Report cards using it will fall back to the default scale.`, [
      { text: 'Cancel', style: 'cancel' },
      { text: 'Delete', style: 'destructive', onPress: async () => {
        try {
  ...
  return (
    <>
      ...
      <FormModal visible={open} title={editing ? 'Edit grading scale' : 'New grading scale'}
        ...
        {bands.map((b, i) => (
          <View key={i} style={styles.bandCard}>
            <View style={styles.bandHead}>
              <Text style={styles.bandIndex}>Band {i + 1}</Text>
              <TouchableOpacity onPress={() => removeBand(i)} hitSlop={10} style={styles.bandDel}>
                <Ionicons name="close-circle" size={20} color={colors.danger} />
              </TouchableOpacity>
            </View>
            <Field label="Grade" value={b.grade}
              onChangeText={(v: string) => setBand(i, 'grade', v)} placeholder="A1" />
            <View style={styles.bandPair}>
              <View style={{ flex: 1 }}>
                <Field label="Min %" value={b.minPercent} keyboardType="numeric"
                  onChangeText={(v: string) => setBand(i, 'minPercent', v)} placeholder="91" />
              </View>
              <View style={{ flex: 1 }}>
                <Field label="Max %" value={b.maxPercent} keyboardType="numeric"
...
const styles = StyleSheet.create({
  ...
  // 36px + hitSlop clears the platform minimum tap target on a phone.
  ...
  bandCard: { backgroundColor: colors.surfaceAlt, borderRadius: radius.md,
    padding: spacing.md, marginTop: spacing.sm },
  bandHead: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' },
  bandIndex: { ...font.label, color: colors.slate, fontWeight: '700' },
